...
//启动连接--》zkserver
// 因为是异步，所以需要同步
void ZookeeperClient::start(NetAddr::s_ptr zookeeper_addr)
{
    zhandle_ = zookeeper_init(zookeeper_addr->ToString().c_str(), global_watcher, 30000, nullptr, nullptr, 0);
    if (zhandle_ == nullptr)
    {
...
void ZookeeperClient::create(const char *path, const char *data, int datalen, int state)
{
    ...
    if (ZNONODE == flag) //不存在
    {
        ...
        {
            LOG_ERROR("falg: %d, znode create error... path: %s", flag, path);
        }
    }
}
...
std::string ZookeeperClient::get_data(const char *path)
{
    ...
    if (flag != ZOK)
    {
        LOG_ERROR("can't get znode... path: %s", path);
        return "";
    }
    else
    {
